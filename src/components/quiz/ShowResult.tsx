...
function ShowResult({ points }: { points: number }) {
  ...
  useEffect(() => {
    const getResult = async () => {
      setLoading(true);
      fetch("/edukacyjne_rew/data/Results.json") //fetching rather than importing for future scaling purposes
        .then((res) => res.json())
        .then((data) => {
          setResults(data);
          setLoading(false);
  ...
  if (loading) {
    return (
      <Icon
        src={CatLoading}
        alt="Here is a cat while we load your result :3"
        sx={{ height: "14rem" }}
      />
    );
  }
